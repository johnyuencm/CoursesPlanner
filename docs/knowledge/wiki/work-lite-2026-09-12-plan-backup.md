...
- `c92ffb9` on `codex/plan-backup`: native JSON download and confirmed restore controls.
...
- `node.exe node_modules/tsx/dist/cli.mjs --test tests/catalog-service.test.ts tests/plan.test.ts tests/scraper.test.ts tests/validation.test.ts` — PASS, 34 tests.
