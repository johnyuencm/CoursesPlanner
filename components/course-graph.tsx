...
function GraphWorkspace() {
  ...
  const [focusCode, setFocusCode] = useState("CS 5500");
  const [selectedCode, setSelectedCode] = useState("CS 5500");
  ...
  const relations = useMemo(() => {
    const result: { source: string; target: string; corequisite: boolean }[] = [];
    const coreqPairs = new Set<string>();
    for (const course of catalog?.courses ?? []) {
      for (const code of course.prerequisiteCodes) result.push({ source: code, target: course.code, corequisite: false });
      for (const code of course.corequisiteCodes) {
        const pair = [course.code, code].sort().join("|");
        if (!coreqPairs.has(pair)) { result.push({ source: code, target: course.code, corequisite: true }); coreqPairs.add(pair); }
      }
    }
    return result;
  }, [catalog]);
  ...
  const visible = useMemo(() => {
    ...
    const maxDepth = depth === "full" ? relations.length + 1 : Number(depth);
    for (let step = 0; step < maxDepth && frontier.length; step++) {
      const next: string[] = [];
      for (const code of frontier) for (const edge of relations) {
        const neighbor = edge.source === code ? edge.target : edge.target === code ? edge.source : null;
        if (neighbor && !distances.has(neighbor)) { distances.set(neighbor, step + 1); next.push(neighbor); }
      }
      frontier = next;
    }
    return distances;
  }, [focusCode, depth, relations]);
  const graph = useMemo(() => {
    ...
    for (const rank of Array.from(columns.keys()).sort((a, b) => a - b)) {
      const codes = columns.get(rank)!.sort();
      const width = Math.ceil(codes.length / 4);
      codes.forEach((code, index) => {
        ...
        nodes.push({ id: code, type: "course", position: { x: (columnX + Math.floor(index / 4)) * 230, y: (index % 4) * 148 + (codes.length === 1 ? 148 : 0) }, data: { code, title: course?.title ?? "Metadata not in this catalog", kind, nodeClass: nodeClassFor(kind === "Locked / external" ? "external" : kind, history.has(code)), credits: course ? `${creditLabel(course)} cr` : "Unknown credits", prerequisites: course ? expressionLabel(course.prerequisites) : "Unknown", corequisites: course ? expressionLabel(course.corequisites) : "Unknown", relation, emphasized: !!relation, focused: code === selectedCode, selectCourse: setSelectedCode, openCourse: (value) => openCourse(value) }, ariaLabel: `${code}: ${course?.title ?? "External reference"}` });
      });
      columnX += width;
    }
    const highlighted = new Set([selectedCode, ...upstream, ...downstream]);
    const edges: Edge[] = relations.filter((edge) => visible.has(edge.source) && visible.has(edge.target)).map((edge, index) => {
    ...
    return { nodes, edges };
  }, [visible, focusCode, selectedCode, upstream, downstream, catalog, courseMap, relations, openCourse, history]);
  if (!catalog) return <CatalogState />;
  const matches = search.trim() ? catalog.courses.filter((course) => `${course.code} ${course.title}`.toLowerCase().replace(/\s/g, "").includes(search.toLowerCase().replace(/\s/g, ""))).slice(0, 8) : [];
